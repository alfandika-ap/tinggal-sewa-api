...
async def main():
    ...
    if os.path.exists("kost.log"):
        os.remove("kost.log")
        print("🗑️ Deleted existing kost.log file")
    ...
    list_page_urls = [
        "https://www.cari-kos.com/search/kos/jawa-timur/kota-malang?price-from=0&price-to=10000000",
        "https://www.cari-kos.com/search/kos/jawa-barat/kota-bandung?price-from=0&price-to=10000000",
        "https://www.cari-kos.com/search/kos/dki-jakarta?price-from=0&price-to=10000000",
        "https://www.cari-kos.com/search/kos/di-yogyakarta?price-from=0&price-to=10000000",
        "https://www.cari-kos.com/search/kos/jawa-timur/kota-surabaya?price-from=0&price-to=10000000",
        # Add more URLs here as needed
    ]

    async with AsyncWebCrawler(config=config) as crawler:
        # Process each list page URL
        for list_page_url in list_page_urls:
            print(f"🔍 Processing list page: {list_page_url}")
            
            try:
                # 1. Scrape halaman list untuk dapatkan list kost dan URL detail-nya
                list_result = await crawler.arun(url=list_page_url)
                
                # Check if page was successfully accessed
                if not list_result or not list_result.markdown:
                    print(f"❌ Cannot access or empty content: {list_page_url}")
                    continue
                
                # Check if page returned error status
                if hasattr(list_result, 'status_code') and list_result.status_code != 200:
                    print(f"❌ HTTP Error {list_result.status_code}: {list_page_url}")
                    continue
                ...
                for kost_summary in kost_list.kosts:
                    ...
                    try:
                        ...
                        # Check if detail page was successfully accessed
                        if not detail_result or not detail_result.markdown:
                            print(f"❌ Cannot access detail page: {kost_summary.url}")
                            continue
                        
                        # Check if detail page returned error status
                        if hasattr(detail_result, 'status_code') and detail_result.status_code != 200:
                            print(f"❌ HTTP Error {detail_result.status_code} for detail: {kost_summary.url}")
                            continue
                        ...
                        if not kost_data.title or not kost_data.title.strip():
                            print(f"❌ Skipping kost with empty title: {kost_summary.url}")
                            continue
                        ...
                        error_msg = str(e)
                        # Skip specific timeout and navigation errors
                        if any(keyword in error_msg.lower() for keyword in [
                            "timeout", "failed on navigating", "page.goto", 
                            "navigation timeout", "net::err", "connection refused",
                            "unexpected error in *crawl*web", "acs-goto"
                        ]):
                            print(f"⏰ Timeout/Navigation error, skipping: {kost_summary.url}")
                            continue
                        
                        print(f"❌ Error scraping detail for {kost_summary.url}: {e}")
                        continue
                        
            except Exception as e:
                error_msg = str(e)
                # Skip specific timeout and navigation errors for list pages too
                if any(keyword in error_msg.lower() for keyword in [
                    "timeout", "failed on navigating", "page.goto", 
                    "navigation timeout", "net::err", "connection refused",
                    "unexpected error in *crawl*web", "acs-goto"
                ]):
                    print(f"⏰ Timeout/Navigation error for list page, skipping: {list_page_url}")
                    continue
                
                print(f"❌ Error processing list page {list_page_url}: {e}")
                continue

    print("🎉 Multi-URL scraping completed!")
